...
class BlogController extends Controller
{
    public function get(Request $request, Blog $blog): JsonResponse {
        if ($blog->id) {
            $blog->load('file.resources', 'categoria');
        ...
        $query = Blog::with('file.resources', 'categoria');
    ...
    public function post(Request $request) {
        $this->validateJSON([
            ...
            'capa_id'           => 'nullable|exists:files,id',
        ...
        $blog->fill($request->json()->all());

        $blog->save();
        $blog->setFriendlyURL(self::slugify($blog->url));

        return response()->json($blog->toArray(), 201);
    }

    public function put(Request $request, Blog $blog): JsonResponse {
        $this->validateJSON([
            'title'             => 'required|string|min:3',
            'text'              => 'nullable',
            'autor'             => 'required|min:3',
    ...
    public function patch(Request $request, Blog $blog): JsonResponse {
        $this->validateJson([
            'name'    => 'required|min:3',
            'file_id' => 'nullable|exists:files,id',
        ]);
