...
<script>
	export default {
		...
		methods: {
			fetch(page = 1) {
				_axios.get(`instagram?page=${page}&per-page=${this.perPage}`)
					.then((response) => {
			...
			deleteSingle(e) {
				...
				$.confirm({
					title: 'Atenção',
					content: `Deseja realmente deletar o instagram <b>${self.items[offset].name}</b>?`,
					type: 'red',
					closeIcon: true,
					useBootstrap: true,
					...
					buttons: {
						confirmar() {
							_axios.delete(`instagram/${self.items[offset].id}`)
								.then(() => {
