                ...
                <b-card>
                    <div slot="header">
                        ...
                        <div class="pull-right actions">
                            <router-link class="d-inline save btn btn-success"
                                         :to="{name: 'Visualizar Acabamentos'}">
                    ...
                    <table class="table table-striped table-default">
                        ...
                        <tr v-for="(data, offset) in items">
                            <td>{{ data.id }}</td>
                            <td>{{ data.name }}</td>
                            <td>{{ data.categoria.name }}</td>
                            <td class="text-center">
                                <router-link class="fa fa-edit fa-lg"
                                             :to="{name: 'Visualizar Acabamentos', params: {target: data.id}}">
                                </router-link>
...
<script>
	export default {
		...
		methods: {
			fetch(page = 1) {
				_axios.get(`acabament?page=${page}&per-page=${this.perPage}`)
					.then((response) => {
			...
			deleteSingle(e) {
				...
				$.confirm({
					title: 'Atenção',
					content: `Deseja realmente deletar a publicação <b>${self.items[offset].name}</b>?`,
					type: 'red',
					closeIcon: true,
					useBootstrap: true,
					...
					buttons: {
						confirmar() {
							_axios.delete(`acabament/${self.items[offset].id}`)
								.then(() => {
