                    ...
                    <div slot="header">
                        ...
                        <div class="pull-right actions">
                            <router-link class="d-inline save btn btn-success"
                                         :to="{name: 'Cadastrar Categorias de Produto'}">
                        ...
                        <tr v-for="(data, offset) in items">
                            ...
                            <td class="text-center">
                                <router-link class="fa fa-edit fa-lg"
                                             :to="{name: 'Cadastrar Categorias de Produto', params: {target: data.id}}">
                                </router-link>
...
<script>
	export default {
		...
		methods: {
			fetch(page = 1) {
				_axios.get(`categories?ativo=1&page=${page}&per-page=${this.perPage}`)
					.then((response) => {
			...
			deleteSingle(e) {
				...
				$.confirm({
					...
					buttons: {
						confirmar() {
							_axios.delete(`categories/${self.items[offset].id}`)
								.then(() => {
