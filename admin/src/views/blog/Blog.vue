                        ...
                        <tr v-for="(data, offset) in items">
                            ...
                            <td class="text-center">
                                <router-link class="fa fa-edit fa-lg"
                                             :to="{name: 'Cadastrar Publicação', params: {target: data.id}}">
                                </router-link>
...
<script>
	export default {
		...
		methods: {
			fetch(page = 1) {
				_axios.get(`blog?page=${page}&per-page=${this.perPage}`)
					.then((response) => {
			...
			deleteSingle(e) {
				...
				$.confirm({
					...
					buttons: {
						confirmar() {
							_axios.delete(`blog/${self.items[offset].id}`)
								.then(() => {
