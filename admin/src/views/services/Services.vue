                        ...
                        <tr v-for="(data, offset) in items">
                            ...
                            <td class="text-center">
                                <router-link class="fa fa-edit fa-lg"
                                             :to="{name: 'Cadastrar Serviços', params: {target: data.id}}">
                                </router-link>
...
<script>
	export default {
		...
		methods: {
			fetch(page = 1) {
				_axios.get(`services?page=${page}&per-page=${this.perPage}`)
					.then((response) => {
			...
			deleteSingle(e) {
				...
				$.confirm({
					title: 'Atenção',
					content: `Deseja realmente deletar o serviço <b>${self.items[offset].title}</b>?`,
					type: 'red',
					closeIcon: true,
					useBootstrap: true,
					...
					buttons: {
						confirmar() {
							_axios.delete(`services/${self.items[offset].id}`)
								.then(() => {
