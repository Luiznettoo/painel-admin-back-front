                ...
                <b-card>
                    ...
                    <table class="table table-striped table-default">
                        ...
                        <tr v-for="(data, offset) in items">
                            <td>{{ data.id }}</td>
                            <td>{{ data.name }}</td>
                            <td><p v-if="data.categoria">{{ data.categoria.name }}</p></td>
                            <td class="text-center">
                                <router-link class="fa fa-edit fa-lg"
                                             :to="{name: 'Cadastrar Opcional', params: {target: data.id}}">
                                </router-link>
...
<script>
	export default {
		...
		methods: {
			fetch(page = 1) {
				_axios.get(`option?page=${page}&per-page=${this.perPage}`)
					.then((response) => {
			...
			deleteSingle(e) {
				...
				$.confirm({
					title: 'Atenção',
					content: `Deseja realmente deletar o opção <b>${self.items[offset].name}</b>?`,
					type: 'red',
					closeIcon: true,
					useBootstrap: true,
					...
					buttons: {
						confirmar() {
							_axios.delete(`option/${self.items[offset].id}`)
								.then(() => {
