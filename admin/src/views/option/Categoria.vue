    ...
    <b-row>
        <b-col cols="12" xl="12">
            <transition name="slide">
                <b-card>
                    <div slot="header">
                        <strong>Categorias de Opções </strong>
                        <small>LISTA</small>

                        <div class="pull-right actions">
                            <router-link class="d-inline save btn btn-success"
                                         :to="{name: 'Cadastrar Grupo de Opcionais'}">
                        ...
                        <tr v-for="(data, offset) in items">
                            ...
                            <td class="text-center">
                                <router-link class="fa fa-edit fa-lg"
                                             :to="{name: 'Cadastrar Grupo de Opcionais', params: {target: data.id}}">
                                </router-link>
...
<script>
	export default {
		...
		methods: {
			fetch(page = 1) {
				_axios.get(`categoriaOption?page=${page}&per-page=${this.perPage}`)
					.then((response) => {
			...
			deleteSingle(e) {
				...
				$.confirm({
					...
					buttons: {
						confirmar() {
							_axios.delete(`categoriaOption/${self.items[offset].id}`)
								.then(() => {
